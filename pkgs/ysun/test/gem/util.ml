let rec pp_value_error ppf = function
  ...
  | Invalid_shape { expected; given } ->
    Format.fprintf ppf "<error-invalid-shape: %s> for %a" expected Yocaml.Data.pp given
...
let print_validated_value pp_ok x =
  print_endline
  @@ Format.asprintf "%a" (Format.pp_print_result ~ok:pp_ok ~error:pp_value_error) x
;;

let make_member ident url =
  let open Yocaml.Data in
  ident, record [ "id", string ident; "main_link", record [ "url", string url ] ]
;;

let make_chain l =
  let chain, members = List.split l in
  Yocaml.Data.Validation.list_of Gem.Model.Member.validate (Yocaml.Data.list members)
