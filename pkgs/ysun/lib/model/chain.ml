...
let unknown_author = Yocaml_syndication.Person.make "Unknown author"
...
let to_authors chain =
  ...
  | [] -> (* Should never happen *) Yocaml.Nel.singleton unknown_author
...
let as_author chain id =
  ...
  |> Option.fold ~none:unknown_author (* Should never happen *) ~some:Member.as_author
;;
