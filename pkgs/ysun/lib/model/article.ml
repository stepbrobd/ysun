...
let validate =
  ...
  record (fun fl ->
    ...
    and+ tags = optional_or fl ~default:[] "tags" @@ list_of Yocaml.Slug.validate
    and+ authors =
      required fl "authors" @@ ((string $ fun x -> [ x ]) / list_of Yocaml.Slug.validate)
    in
...
let authors_in_chain chain { authors; _ } =
  ...
  List.for_all
    (fun author -> chain |> List.exists (fun (x, _) -> String.equal author (Member.id x)))
    authors
;;
...
let pp ppf article =
  Format.fprintf ppf "%a" Yocaml.Data.pp (article |> normalize |> Yocaml.Data.record)
;;
