...
let validate =
  ...
  record (fun fields ->
    ...
    and+ interests = optional_or ~default:[] fields "interests" (list_of Link.validate) in
    { page; chain = Chain.empty; interests })
;;

let merge_chain chain =
  Yocaml.Task.lift ~has_dynamic_dependencies:false (fun index -> { index with chain })
;;
