...
let normalize_underlying_link (title, lang, url) =
  ...
  [ "title", string title; "lang", Lang.normalize lang; "url", Url.normalize url ]
;;
...
let normalize_to_semantic_list links =
  ...
  |> List.mapi (fun i link ->
    let sep = if i >= len - 1 then "" else if i >= len - 2 then " and " else ", " in
...
let equal (title_a, lang_a, url_a) (title_b, lang_b, url_b) =
  String.equal title_a title_b && Lang.equal lang_a lang_b && Url.equal url_a url_b
;;
