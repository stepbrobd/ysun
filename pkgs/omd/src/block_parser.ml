...
module Pre = struct
  ...
  let rec process link_defs { blocks; next } s =
    ...
    | Rparagraph lines, _ -> { blocks; next = Rparagraph (StrSlice.to_string s :: lines) }
    ...
    | Rfenced_code (ind, num, q, info, lines, a), _ ->
      ...
      in
      { blocks
      ; next = Rfenced_code (ind, num, q, info, StrSlice.to_string s :: lines, a)
      }
    ...
    | Rhtml ((Hcontains l as k), lines), _
      ...
      { blocks = close { blocks; next = Rhtml (k, StrSlice.to_string s :: lines) }
      ; next = Rempty
      }
    | Rhtml (Hblank, _), Lempty -> { blocks = close { blocks; next }; next = Rempty }
    | Rhtml (k, lines), _ -> { blocks; next = Rhtml (k, StrSlice.to_string s :: lines) }
      ...
      let rec loop = function
        ...
        | Rparagraph (_ :: _ as lines) ->
          (match classify_line s with
           ...
           | Lhtml (false, _) -> Some (Rparagraph (StrSlice.to_string s :: lines))
           | _ -> None)
        | _ -> None
      in
      (match loop next with
