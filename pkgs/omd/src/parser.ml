...
let thematic_break =
  ...
  fun s ->
    ...
    | Some (('*' | '_' | '-') as symb) ->
      if StrSlice.fold_left (f symb) 0 s >= 3
      then
...
let setext_heading s =
  ...
  let level, symb =
    ...
    | _ -> raise Fail
  in
  let heading_chars, rest = StrSlice.split_at (fun c -> not (Char.equal c symb)) s in
  let len = StrSlice.length heading_chars in
  if Char.equal symb '-' && len = 1
  then
    (* can be interpreted as an empty list item *)
    raise Fail
  else if not (StrSlice.for_all is_whitespace rest)
  then
        ...
        | Some '}' ->
          ...
          (match StrSlice.head s with
           | None ->
             ...
             StrSlice.of_string (Buffer.contents buf), Some (Buffer.contents buf')
           | Some _ ->
...
let entity s =
  ...
  | '#' :: ('x' | 'X') :: _ ->
    let rec loop m n s =
      ...
      match StrSlice.head s with
      | Some ('a' .. 'f' as c) ->
        loop (succ m) ((n * 16) + Char.code c - Char.code 'a' + 10) (StrSlice.tail s)
      | Some ('A' .. 'F' as c) ->
        loop (succ m) ((n * 16) + Char.code c - Char.code 'A' + 10) (StrSlice.tail s)
      | Some ('0' .. '9' as c) ->
        loop (succ m) ((n * 16) + Char.code c - Char.code '0') (StrSlice.tail s)
      | Some ';' ->
  ...
  | '#' :: _ ->
    let rec loop m n s =
      ...
      match StrSlice.head s with
      | Some ('0' .. '9' as c) ->
        loop (succ m) ((n * 10) + Char.code c - Char.code '0') (StrSlice.tail s)
      | Some ';' ->
        if m = 0 then raise Fail;
        let u = if n = 0 || not (Uchar.is_valid n) then Uchar.rep else Uchar.of_int n in
        [ u ], StrSlice.tail s
  ...
  | ('a' .. 'z' | 'A' .. 'Z') :: _ ->
    let rec loop len t =
      ...
      | Some ('a' .. 'z' | 'A' .. 'Z' | '0' .. '9') -> loop (succ len) (StrSlice.tail t)
      | Some ';' ->
...
let unordered_list_item ind s =
  ...
  | Some (('+' | '-' | '*') as c) ->
    let s = StrSlice.tail s in
    if is_empty s
    then Llist_item (Bullet c, 2 + ind, s)
    else (
      let n = indent s in
...
let ordered_list_item ind s =
  let rec loop n m s =
    ...
    | Some (('.' | ')') as c) ->
      let s = StrSlice.tail s in
      if is_empty s
      then Llist_item (Ordered (m, c), n + 1 + ind, s)
      else (
        let ind' = indent s in
...
let def_list s =
  let s = StrSlice.tail s in
  match StrSlice.head s with
  | Some w when is_whitespace w -> Ldef_list (String.trim (StrSlice.to_string s))
  | _ -> raise Fail
;;
...
let table_row ~pipe_prefix s =
  let rec loop items seen_pipe s =
    ...
    | None ->
      if StrSlice.for_all is_whitespace s then items, seen_pipe else s :: items, false
    | Some i ->
  ...
  let rec gobble_open_backtick n =
    ...
    | Some _ ->
      ...
      let finish () =
        ...
        let content =
          if String.for_all (fun c -> c = ' ') content
          then content
          else if
              ...
              | label_text ->
                ...
                (match
                   List.find_opt
                     ...
                     defs
                 with
                 | Some { destination; title; attributes = attr; _ } ->
                   let def = { label; destination; title } in
                   let r =
                     match k with
                     | Img -> Image (attr, def)
