...
let rec drop_while f s =
  match uncons s with
  | Some (x, s') when f x -> drop_while f s'
  | _ -> s
;;
...
let index_unescaped sep s =
  let rec loop idx state =
    if idx = s.off + s.len
    then None (* If we get here and we're inside a verbatim span, what to do? *)
    else (
...
let exists f s =
  let rec loop s i =
    if i >= s.len then false else if f s.base.[s.off + i] then true else loop s (succ i)
  in
  loop s 0
;;
