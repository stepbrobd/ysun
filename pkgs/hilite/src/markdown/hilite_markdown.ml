let transform ?(skip_unknown_languages = true) ?lookup_method ?tm (doc : Cmarkit.Doc.t) =
  let block _mapper (b : Cmarkit.Block.t) =
    match b with
    | Cmarkit.Block.Code_block (node, meta) ->
      let info = Cmarkit.Block.Code_block.info_string node |> Option.map fst in
      let code =
        Cmarkit.Block.Code_block.code node
        |> List.map Cmarkit.Block_line.to_string
        |> String.concat "\n"
      in
      (match Option.bind info Cmarkit.Block.Code_block.language_of_info_string with
       | Some (lang, _) ->
